...
/** Create SweetAppleController. */
SweetAppleController::SweetAppleController(Main* main_)
{
...
/** Get a button. */
Button& SweetAppleController::getButton(enum ButtonID id)
{
  return buttons[id];
}
...
/** Is a button pressed? */
bool SweetAppleController::buttonPressed(enum ButtonID button)
{
  return buttons[button].pressed();
}

/** Value of the wheel. */
int SweetAppleController::wheelValue()
{
  return wheel.value();
}

/** Change value of the wheel. */
void SweetAppleController::addToWheel(const int i)
{
  if(i < 0)
  {
                                             ...
                                             QDialog(parent, name, f)
{
  ...
  (
    ...
    "        <tr><td>Return</td><td>Action</td></tr>"
    ...
    "  a value. Pressing <b>Shift</b> button together with rotation buttons"
    "  multiplies value change by 10. Pressing <b>Shift</b> button together"
    "  with <b>Play</b> button pauses and unpauses the current patch.<br/>"
    "  This program also understands the turning gesture: Turning the mouse cursor"
    "  around the center of the program's window pushing the (left) mouse button"
    "  means the same as using rotation buttons (<b>Up</b> and <b>Down</b>);"
    "  using a touchscreen instead of a mouse achieves the same effect.<br/>"
